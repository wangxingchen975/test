...
class HandleLagouData(object):
    ...
    def query_salary_result(self):
        ...
        # print(result_list2)
        result = [{"name": x[0], "value": x[1]} for x in result_list2 if x[1]>10]
    ...
    def query_job_result(self):
        info = {}
        result = self.mysql_session.query(Lagoutables.crawl_date,func.count('*').label('c')).group_by(Lagoutables.crawl_date).order_by(Lagoutables.crawl_date).all()
...
if __name__ == '__main__':
    lagou_mysql = HandleLagouData()
    for x in lagou_mysql.query_jobNature_result()["data"]:
        print(x)
